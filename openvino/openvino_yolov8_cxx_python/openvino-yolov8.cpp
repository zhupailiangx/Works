...
int main(int argc, char* argv[])
{
    ...
    auto compiled_model = core.compile_model("../yolov8n.onnx", "CPU"//"CPU"
    );
    ...
    for (int i = 0; i < 100; i++) {
        ...
        //毫秒级
        double duration_millsecond = std::chrono::duration<double, std::milli>(e - s).count();
        //std::cout << duration_millsecond << "ms" << std::endl;

        total_fps += (1000 / duration_millsecond);

        auto fps = (1000 / duration_millsecond);
        std::stringstream fps_label;
        fps_label << "FPS: ";
        fps_label << fps;
        //fps_label << "FPS";
        
        putText(img, fps_label.str(), Point(10, 25), FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2);
    ...
    std::cout << "Mean FPS: " << total_fps/100<< std::endl;
    return 0;
}
